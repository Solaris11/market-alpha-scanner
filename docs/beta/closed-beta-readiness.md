...
- `TRADEVETO_BETA_PREMIUM_ACCESS=false`: optional emergency opt-out for the automatic beta-premium entitlement. In invite mode with an invite code configured, authenticated active users are treated as closed-beta premium users without creating Stripe subscriptions.
