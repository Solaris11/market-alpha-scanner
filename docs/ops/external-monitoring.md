...
Provision the required TradeVeto monitors with the official UptimeRobot API:
...
tools/ops/tradeveto-uptimerobot-setup.sh
```

Required URLs:

- `https://tradeveto.com`
- `https://tradeveto.com/pricing`
...
- `SLACK_WEBHOOK_URL`
- `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` or `TRADEVETO_ALERT_TELEGRAM_CHAT_ID`
- SMTP email: `SMTP_HOST`, `SMTP_PORT`, `SMTP_SECURE`, `SMTP_USER`, `SMTP_PASS`, `EMAIL_FROM`, and optional `TRADEVETO_ALERT_EMAIL_TO`
