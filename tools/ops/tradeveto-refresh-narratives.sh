...
docker run --rm \
  ...
  -w /work \
  "${NODE_IMAGE}" \
  sh -lc 'export DATABASE_URL="${FRONTEND_DATABASE_URL:-postgresql://${POSTGRES_USER:-market_alpha}:${POSTGRES_PASSWORD:?missing}@market-alpha-postgres:5432/${POSTGRES_DB:-market_alpha}}"; npm run narrative:refresh'
