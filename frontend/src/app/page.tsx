...
function countByDecision(rows: { final_decision?: string }[], decision: string) {
  ...
  return (
    <section className={`rounded-md border px-3 py-2 ${regimeTone(label)}`}>
      <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
        ...
        <div className="text-xs opacity-75">Used to shape risk posture and entry aggressiveness.</div>
      </div>
    </section>
  );
}
      ...
      <div className="space-y-3">
        <MetricStrip
          metrics={[
            ...
            { label: "Avg Score", value: ranking.length ? formatNumber(averageScore) : "N/A", meta: "current universe" },
            { label: "ENTER", value: countByDecision(ranking, "ENTER"), meta: "trade-ready" },
            { label: "WAIT", value: countByDecision(ranking, "WAIT_PULLBACK"), meta: "pullback" },
            { label: "WATCH", value: countByDecision(ranking, "WATCH") || countBy(ranking, "WATCH"), meta: "monitor" },
            { label: "AVOID", value: countByDecision(ranking, "AVOID"), meta: "blocked" },
            { label: "EXIT", value: countByDecision(ranking, "EXIT"), meta: "risk-off" },
