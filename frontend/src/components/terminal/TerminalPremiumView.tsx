...
import { buildOpportunitiesPageModel, stripNarrativeForTerminal, stripRawFields, stripShockEventsForClient, stripShockPatternForTerminal } from "@/lib/trading/opportunity-view-model";
...
type TerminalEntitlement = Awaited<ReturnType<typeof getEntitlement>>;

/**
 * The authenticated terminal, split out of the route so its client components
 * form their own chunk group.
 *
 * Anonymous visitors hit /terminal to find a locked preview, but every client
 * component in this tree used to sit in the route's initial bundle: 22 chunks
 * and 1974 KB of JavaScript to render a sign-in wall. Loading this view through
 * next/dynamic means that payload is fetched only when someone with premium
...
export async function TerminalPremiumView({ entitlement }: { entitlement: TerminalEntitlement }) {
  ...
  const clientRows = timeline.sync("stripForClient", () => stripShockPatternForTerminal(stripNarrativeForTerminal(stripRawFields(stripShockEventsForClient(opportunityModel.rows)))),
