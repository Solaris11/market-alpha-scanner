...
import { Bell, CalendarDays, ChevronRight, Clock, Inbox, Mail, Radio, Settings, Smartphone } from "lucide-react";
    ...
    <GlassPanel className="poster-scanline overflow-hidden border-sky-300/15 bg-sky-400/[0.025] p-4 sm:p-5" id="intelligence-feed">
      <div className="flex flex-col gap-4 xl:flex-row xl:items-start xl:justify-between">
        <div className="min-w-0">
          ...
          <h2 className="mt-2 text-2xl font-semibold tracking-tight text-slate-50">Daily brief and high-signal updates</h2>
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-3">
          ...
          <div className="mt-3 grid gap-2">
            {brief.bullets.map((bullet) => (
              <div className="flex gap-2 rounded-xl border border-white/10 bg-white/[0.025] p-3 text-sm leading-5 text-slate-300" key={bullet}>
                <span className="mt-1 h-2 w-2 shrink-0 rounded-full bg-cyan-300 shadow-[0_0_12px_rgba(34,211,238,0.5)]" />
                <span>{bullet}</span>
              </div>
            ))}
          </div>
          <div className="mt-3 rounded-2xl border border-cyan-300/15 bg-cyan-300/[0.045] p-3">
            <div className="text-[10px] font-black uppercase tracking-[0.18em] text-cyan-200">What to monitor</div>
        ...
        <div className="rounded-2xl border border-white/10 bg-slate-950/35 p-3">
          ...
          <div className="mt-3 grid gap-2">
            {visibleItems.length ? visibleItems.map((item) => (
              <Link
                className={`group rounded-2xl border p-3 transition hover:-translate-y-0.5 hover:bg-white/[0.045] ${itemClass(item)}`}
              ...
              >
                <div className="flex min-w-0 items-start justify-between gap-3">
                  ...
                  <ChevronRight className="mt-1 h-4 w-4 shrink-0 text-slate-600 transition group-hover:translate-x-0.5 group-hover:text-cyan-200" />
                </div>
                <div className="mt-3 flex flex-wrap gap-2 text-[11px]">
                  {item.relatedSymbol ? <span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-2 py-1 font-mono font-black text-cyan-100">{item.relatedSymbol}</span> : null}
                  {item.notificationEligible ? <span className="rounded-full border border-emerald-300/20 bg-emerald-300/10 px-2 py-1 font-semibold text-emerald-100">notification eligible</span> : <span className="rounded-full border border-white/10 bg-white/[0.03] px-2 py-1 text-slate-500">feed only</span>}
                </div>
              </Link>
            )) : (
