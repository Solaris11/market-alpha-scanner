...
export function SupportTicketForm({ anonymous = false }: { anonymous?: boolean }) {
  ...
  return (
    <form action={submit} className="grid gap-3">
      {anonymous ? <input className="rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-slate-100 outline-none ring-1 ring-white/5 transition focus:border-cyan-300/45 focus:ring-cyan-300/20" name="email" placeholder="Email" required type="email" /> : null}
      <input className="rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-slate-100 outline-none ring-1 ring-white/5 transition focus:border-cyan-300/45 focus:ring-cyan-300/20" maxLength={180} name="subject" placeholder="Subject" required />
      <select className="rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-slate-100 outline-none ring-1 ring-white/5 transition focus:border-cyan-300/45 focus:ring-cyan-300/20" name="category">
        {categories.map((category) => <option key={category} value={category}>{humanizeLabel(category)}</option>)}
      </select>
      <textarea className="min-h-36 rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-slate-100 outline-none ring-1 ring-white/5 transition focus:border-cyan-300/45 focus:ring-cyan-300/20" maxLength={4000} name="message" placeholder="How can we help?" required />
      <button className="w-fit rounded-full border border-cyan-300/35 bg-cyan-400/10 px-4 py-2 text-sm font-black text-cyan-100 shadow-[0_0_24px_rgba(34,211,238,0.12)] transition hover:border-cyan-200/70 hover:bg-cyan-400/15" type="submit">Send support request</button>
...
export function SupportReplyForm({ ticketId }: { ticketId: string }) {
  ...
  return (
    <form action={submit} className="grid gap-3">
      <textarea className="min-h-28 rounded-2xl border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-slate-100 outline-none ring-1 ring-white/5 transition focus:border-cyan-300/45 focus:ring-cyan-300/20" name="message" placeholder="Reply" required />
      <button className="w-fit rounded-full border border-cyan-300/35 bg-cyan-400/10 px-4 py-2 text-sm font-black text-cyan-100" type="submit">Reply</button>
      {status ? <p className="text-sm text-slate-400">{status}</p> : null}
    </form>
  );
}
...
export function SupportChatBox() {
  ...
  return (
    <div className="grid gap-4">
      <div className="poster-panel grid gap-3 rounded-3xl border border-cyan-300/15 bg-cyan-400/[0.04] p-4 md:grid-cols-[1.1fr_0.9fr]">
        <div>
      ...
      <div className="grid max-h-[520px] gap-3 overflow-y-auto rounded-3xl border border-white/10 bg-white/[0.03] p-4">
      ...
      <form action={submit} className="flex flex-col gap-3 sm:flex-row">
        <input className="min-w-0 flex-1 rounded-full border border-white/10 bg-slate-950/80 px-4 py-3 text-sm text-slate-100 outline-none ring-1 ring-white/5 transition focus:border-cyan-300/45 focus:ring-cyan-300/20" disabled={busy} name="message" onChange={(event) => setDraft(event.target.value)} placeholder="Ask about scanner states, alerts, billing, or troubleshooting" value={draft} />
        <button className="rounded-full border border-cyan-300/35 bg-cyan-400/10 px-5 py-3 text-sm font-black text-cyan-100 disabled:opacity-50" disabled={busy} type="submit">Ask</button>
      </form>
    </div>
  );
}
